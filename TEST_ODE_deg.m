...
function Test_isConstant(Y0,Time,options, P, ODE, name, testnr)
    P0 = paramIL10_deg;
    [~,Y] = ode15s( @(t,y) ODEs_IL10_deg(t,y,P0,[]), Time, Y0, options);
    ...
    figure();
    plot_time = Time/3600;
    plot(plot_time,all_ODE)
    title(['The sum of all ',name,' stays constant'], 'FontSize', 14)
...
function Test_7_PPX_suppreses_STAT3cpd_PPXbindingConstant(Y0,Time,options, N)
    ...
    [~,Y]=ode15s( @(t,y) ODEs_IL10_deg(t,y,P0,ii) ,Time,Y0,options);
...
function Test_10_NoDeDimerisation_STAT3nd_more_mRNA(Y0,Time,options,N)
    ...
    [~,Y]=ode15s( @(t,y) ODEs_IL10_deg(t,y,P0,[]) ,Time,Y0,options);
    ii = int64(10);
    [~,Y2]=ode15s( @(t,y) ODEs_IL10_deg(t,y,P0,ii) ,Time,Y0,options);
...
function Test_11_NoPPX_more_mRNA(Y0,Time,options,N)
    ...
    Y0(5)=0;
    [~,Y2]=ode15s( @(t,y) ODEs_IL10_deg(t,y,P0,[]) ,Time,Y0,options);
